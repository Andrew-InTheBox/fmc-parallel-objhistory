...
config_df = pd.read_csv(path_to_mtd / "ConfigurationFileExample.csv")
task_groups = {}
default_group = "DEFAULT_GROUP"

# Initialize tasks dict with fmc_mtd
tasks = {"fmc_mtd": fmc_mtd}

# Create group completion markers
group_completion_tasks = {}

# First pass: Create all tasks and organize them into groups
for map, info in mappings.items():
    ...
    # Find task's group from configuration
    task_config = config_df[config_df['Task'] == map]
    group = default_group
    if not task_config.empty and pd.notna(task_config['FMC flow target'].iloc[0]):
        group = task_config['FMC flow target'].iloc[0]
    
    # Add task to group
    if group not in task_groups:
        task_groups[group] = []
        # Create group completion marker
        group_completion_tasks[group] = DummyOperator(
            task_id=f"{group}_complete",
            dag=RAW_BUSINESS_VAULT_INIT
        )
...
for map, info in mappings.items():
    ...
    # Connect task to group completion marker
    group_completion_tasks[group] << task

# Create end task and connect group completion markers to it
end_task = DummyOperator(
    task_id="end_analyse",
    dag=RAW_BUSINESS_VAULT_INIT
)

# Connect all group completion markers to end task
for completion_task in group_completion_tasks.values():
    end_task << completion_task

# Connect fmc_mtd to all tasks that depend on it
for group_tasks in task_groups.values():
    for task in group_tasks:
        if 'fmc_mtd' in mappings[task.task_id]['dependencies']:
            task << fmc_mtd
