CREATE OR REPLACE PROCEDURE "ColruytFMC_PROC"."FMC_UPD_RUN_STATUS_FL_DTA"(P_LOAD_CYCLE_ID VARCHAR2,
P_SUCCESS_FLAG VARCHAR2,
P_UPD_ALL_OBJECTS VARCHAR2)
 RETURNS varchar 
LANGUAGE JAVASCRIPT 
...
var HIST_UPD = snowflake.createStatement( {sqlText: `
	UPDATE "ColruytFMC_FMC"."FMC_LOADING_HISTORY" "HIST_UPD"
	SET 
...
var OBJ_HIST_UPD = snowflake.createStatement( {sqlText: `
	...
	FROM 	( 
		...
		FROM "TEST_FMC_MTD"."LOAD_CYCLE_INFO" "LCI_SRC"
		WHERE  '` + P_UPD_ALL_OBJECTS + `' = 'Y'
	) "LCIV"
	WHERE "HIST_UPD"."LOAD_CYCLE_ID" =  "LCIV"."LOAD_CYCLE_ID"
	;
`} ).execute();
