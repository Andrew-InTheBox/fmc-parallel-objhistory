CREATE OR REPLACE PROCEDURE "ColruytFMC_PROC"."SAT_DTA_PRODUCTS_INCR"()
 RETURNS varchar 
LANGUAGE JAVASCRIPT 
...
var truncate_SAT_TEMP_TGT = snowflake.createStatement( {sqlText: `
	TRUNCATE TABLE "TEST_FMC_STG"."SAT_DTA_PRODUCTS_TMP";
`} ).execute();


var SAT_TEMP_TGT = snowflake.createStatement( {sqlText: `
	INSERT INTO "TEST_FMC_STG"."SAT_DTA_PRODUCTS_TMP"(
		 "PRODUCTS_HKEY"
		,"LOAD_DATE"
		,"LOAD_CYCLE_ID"
		,"RECORD_TYPE"
	...
	( 
		...
		FROM "TEST_FMC_STG"."PRODUCTS" "STG_DIS_SRC"
	...
	( 
		SELECT 
			...
			, 'STG' AS "SOURCE"
			, 1 AS "ORIGIN_ID"
			, UPPER(SHA1_HEX(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( "STG_TEMP_SRC"."PRODUCT_NAME"),'~'),'\\#','\\\\' || 
				'\\#'))|| '\\#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_TEMP_SRC"."CATEGORY"),'~'),'\\#','\\\\' || '\\#'))|| '\\#' ||  UPPER(REPLACE(COALESCE(TRIM( TO_CHAR("STG_TEMP_SRC"."BASE_PRICE")),'~'),'\\#','\\\\' || '\\#'))|| '\\#','\\#' || '~'),'~') )) AS "HASH_DIFF"
			...
			, "STG_TEMP_SRC"."BASE_PRICE" AS "BASE_PRICE"
			, "STG_TEMP_SRC"."CATEGORY" AS "CATEGORY"
		FROM "TEST_FMC_STG"."PRODUCTS" "STG_TEMP_SRC"
		...
		SELECT 
			...
			, MAX("SAT_SRC"."LOAD_DATE")OVER( PARTITION BY "SAT_SRC"."PRODUCTS_HKEY") AS "LOAD_END_DATE"
			, 'SAT' AS "RECORD_TYPE"
			, 'SAT' AS "SOURCE"
			, 0 AS "ORIGIN_ID"
			...
			, "SAT_SRC"."BASE_PRICE" AS "BASE_PRICE"
			, "SAT_SRC"."CATEGORY" AS "CATEGORY"
		FROM "DV_FL"."SAT_DTA_PRODUCTS" "SAT_SRC"
	...
	SELECT 
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND "TEMP_TABLE_SET"."DELETE_FLAG"::varchar || "TEMP_TABLE_SET"."HASH_DIFF"::varchar =
			LAG( "TEMP_TABLE_SET"."DELETE_FLAG"::varchar || "TEMP_TABLE_SET"."HASH_DIFF"::varchar,1)OVER(PARTITION BY "TEMP_TABLE_SET"."PRODUCTS_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"
		...
		, "TEMP_TABLE_SET"."BASE_PRICE" AS "BASE_PRICE"
		, "TEMP_TABLE_SET"."CATEGORY" AS "CATEGORY"
...
var SAT_INUR_TGT = snowflake.createStatement( {sqlText: `
	...
	SELECT 
		...
		, "SAT_TEMP_SRC_INUR"."BASE_PRICE" AS "BASE_PRICE"
		, "SAT_TEMP_SRC_INUR"."CATEGORY" AS "CATEGORY"
	FROM "TEST_FMC_STG"."SAT_DTA_PRODUCTS_TMP" "SAT_TEMP_SRC_INUR"
